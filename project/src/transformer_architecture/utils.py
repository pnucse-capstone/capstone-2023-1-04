...
def load_dataset(mode):
    ...
    if mode == 'train':
        train_file = os.path.join(data_dir, 'our_sam_train.csv')
        train_data = pd.read_csv(train_file, encoding='utf-8')

        valid_file = os.path.join(data_dir, 'our_sam_val.csv')
        ...
        test_file = os.path.join(data_dir, 'our_sam_test.csv')
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    file_kor = open('data/pickles/kor.pickle', 'rb')
    kor = pickle.load(file_kor)

    file_eng = open('data/pickles/eng.pickle', 'rb')
...
def display_attention(candidate, translation, attention):
    ...
    font_location = 'data/pickles/NanumSquareR.ttf'
